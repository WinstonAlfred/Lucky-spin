...
import { SelectMacbookWinner, getParticipants } from '@/actions/participant';
...
export function MacbookLotterySection() {
  const [macbookWinner, setMacbookWinner] = useState<Participant | null>(null);
  ...
  return (
      <div className="max-w-2xl mx-auto space-y-8 p-4">
        ...
        <Button 
          ...
          disabled={isSelectingMacbook || participants.length === 0}
        ...
        >
          {isSelectingMacbook ? '🎰 Rolling the Lucky Draw...' : '🎰 Start Lucky Draw'}
        </Button>
  
        {macbookWinner && !isSelectingMacbook && (
          <Card className="border-4 border-purple-400 transform hover:scale-105 transition-transform shadow-xl">
            <CardContent className="pt-6">
              <div className="text-center space-y-4">
                ...
                <p className="text-4xl font-bold text-blue-600">{macbookWinner.name}</p>
                <p className="text-lg text-gray-600">You've won a Macbook AIR M1! 📱</p>
              </div>
            </CardContent>
          </Card>
        )}
  ...
  export default MacbookLotterySection;